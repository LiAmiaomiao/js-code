...
function importFromBelow(){
    ...
    function reconcile(parentDom, instance, element){
        if(instance == null){
            ...
            &&newInstance.publicInstance.componentWillMount();
            parentDom.appentChild(newInstance.dom);
            //componentDidMount
            newInstance.publicInstance
            ...
            //componentWillUnmount
            newInstance.publicInstance
            &&newInstance.publicInstance.componentWillUnmount
            &&newInstance.publicInstance.componentWillUnmount();
    ...
    function instantiate(element){
        ...
        const isClassElement = !!(type.prototype&&type.prototype.isReactComonent);
        if(isDomElement){
            //创建dom
            const isTextElement = type=== TEXT_ELEMENT;
            const dom = isTextElement ? document.createTextNode('') : docuement.createElement(type);
            //设置dom的事件，数据属性
            updateDomPropertries(dom, [], element.props);
            const children = props.children || [];
            const childInstances = children.map(instantiate);
            const childDoms = childInstances.map(childInstance=>childInstcance.dom);
